...
final class ThemeManager: ObservableObject {
    ...
    var accent: Color {
        mode == .kurokula ? Theme.accent : .accentColor
    }

    /// Secondary highlight (neon yellow in Kurokula).
    var secondaryAccent: Color {
        mode == .kurokula ? Theme.secondary : .accentColor
    }

    var success: Color {
        mode == .kurokula ? Theme.success : .green
    }

    var error: Color {
        mode == .kurokula ? Theme.error : .red
    }

    /// Base background used in hero gradients.
    var heroBackground: Color {
        mode == .kurokula ? Theme.background : Color(nsColor: .windowBackgroundColor)
    }
}
